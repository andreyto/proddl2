/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
 * gdiam.h -
 *     Implmeents an algorithm for computing a diameter, and
 *     tightfitting bounding box of a point-set.
 * 
...
template<int n_dim> struct gdiam_point_type {
  gdiam_real a[n_dim];
  gdiam_real& operator[](int i) { return a[i]; }
  const gdiam_real& operator[](int i) const { return a[i]; }
};

//typedef  gdiam_point_type<GDIAM_DIM> gdiam_point_t;
//typedef  gdiam_point_type<2>         gdiam_point_2d_t;
...
inline  void  pnt_copy( gdiam_point  dst, 
                        const gdiam_point  src ) {
    dst[ 0 ] = src[ 0 ]; 
    dst[ 1 ] = src[ 1 ];
    dst[ 2 ] = src[ 2 ];
}
...
public:
    ...
    void  set_third_dim_longest() {
        ...
        if  ( ( high_1 - low_1 ) > ( high_3 - low_3 ) ) {
            ...
            pnt_copy(pnt_tmp,dir_1);
            pnt_copy(dir_1,dir_3);
    ...
    void  get_vertex( double  sel1, double  sel2, double  sel3 , gdiam_real* res) const {
        ...
        gdiam_point_t&  pnt = *((gdiam_point_t*)res);
        pnt_zero( pnt );
	...
	get_vertex(sel1,sel2,sel3,(gdiam_real*)&pnt);
