/**
AT: removed gzip support to avoid problems with boost::iostreams missing
boost_gzip library.
/**
...
std::string bob::core::tmpdir() {
	//This method is not yet available in Boost 1.44 filesystem v3 API
  //return boost::filesystem::temp_directory_path.native();
  return boost::filesystem::current_path().string();
}

std::string bob::core::tmpfile(const std::string& extension) {
  return (boost::filesystem::path(tmpdir())/boost::filesystem::unique_path()).string() + 
	  extension;
}
